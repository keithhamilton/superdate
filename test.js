...
test('resolve day and month to names', function (t) {
  ...
  var date = new Date(2015, 0, 1); // Jan. 1, 2015
...
test('increment month', function (t) {
  ...
  var date = new Date(2013, 0, 1); // Jan. 1, 2015
...
test('increment minutes', function (t) {
  ...
  var date = new Date(2015, 0, 1, 12, 30);
...
test('increment seconds', function (t) {
  ...
  var date = new Date(2015, 0, 1, 12, 59, 59);
...
test('increment milliseconds', function (t) {
  ...
  var date = new Date(2015, 0, 1, 12, 59, 59, 999);
...
test('get first day of month', function (t) {
  t.plan(1);
  var testDate = new Date(2015, 0, 3); // first Saturday of 2015 was the third

  t.equal(superdate.firstDayInstance(6, 0, 2015).getTime(), testDate.getTime(), 'should equal the first saturday in 2015');
});
...
test('test translation from text', function (t) {
  ...
  var twoDaysFuture = superdate.dateFromSpeech(currentDate, 'in two days');
  t.equal(Math.floor((twoDaysFuture - currentDate) / 86400000), 2);

  var threeMonthsFuture = superdate.dateFromSpeech(currentDate, 'in three months');
  t.equal(Math.round((threeMonthsFuture - currentDate) / 86400000 / 10) * 10, 90);

  var threefoursevenDays = superdate.dateFromSpeech(currentDate, 'in three hundred forty seven days');
  t.equal(Math.floor((threefoursevenDays - currentDate) / 86400000), 347);

  var threefoursevenDaysAgo = superdate.dateFromSpeech(currentDate, 'three hundred forty seven days ago');
  t.equal(Math.floor((threefoursevenDaysAgo - currentDate) / 86400000), -347);

  var janOne = new Date(2015, 0, 1); // January 1, 2015 was a Thursday
  var nextWednesday = superdate.dateFromSpeech(janOne, 'next wednesday');
  t.equal(nextWednesday.getDate(), 7, 'should be the seventh of January');

  var janSeven = new Date(2015, 0, 7); // January 7, 2015 was a Wednesday
  var lastThursday = superdate.dateFromSpeech(janSeven, 'last thursday');
  t.equal(lastThursday.getDate(), 1, 'should be the first of January');

  var minusTwenty = superdate.dateFromSpeech(currentDate, 'minus twenty days');
  t.equal(Math.round((currentDate - minusTwenty) / 86400000), 20);

  var zeroDays = superdate.dateFromSpeech(currentDate, 'zero days from now');
...
test('get date of first instance of day [in month] [in year]', function (t) {
  t.plan(2);
  var dayDate = superdate.firstDayInstance(0, 0, 2014);
  t.equal(dayDate.getDate(), 5, 'should be 5 for first Sunday in Jan/2014');

  var nextDayDate = superdate.firstDayInstance(5, 0, 2014);
...
test('get date of last instance of day [in month] [in year]', function (t) {
  t.plan(2);
  var dayDate = superdate.lastDayInstance(0, 0, 2014);
  t.equal(dayDate.getDate(), 26, 'should be 26 for last Sunday in Jan/2014');

  var nextDayDate = superdate.lastDayInstance(5, 0, 2014);
...
test('get nth instance of day [in month] [in year]', function (t) {
  ...
  var date = new Date();
  /* 
     methods with omitted parameters commented out for purposes of showing how 
     the method works. To test, you will need to update the dates to match the 
     current month/year
  */
  var day = superdate.nthDayInstance(new Date(2015, 0), 0, 1);
...
test('get next instance of day', function (t) {
  t.plan(2);
  var date = new Date(2015, 0, 1); // January 1, 2015
