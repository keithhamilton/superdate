...
test('resolve day and month to names', function(t){
  ...
  const date = new Date(2015,0,1) // Jan. 1, 2015
...
test('increment month', function(t) {
  ...
  const date = new Date(2013,0,1) // Jan. 1, 2015
...
test('increment hour', function(t) {
  ...
  const date = new Date(2015, 0, 1, 12)
...
test('increment minutes', function(t) {
  ...
  const date = new Date(2015, 0, 1, 12, 30);
...
test('increment seconds', function(t) {
  ...
  const date = new Date(2015, 0, 1, 12, 59, 59);
...
test('increment milliseconds', function(t) {
  ...
  const date = new Date(2015, 0, 1, 12, 59, 59, 999);
...
test('get first day of month', function(t) {
  t.plan(1)
  const testDate = new Date(2015, 0, 3) // first Saturday of 2015 was the third
...
test('test translation from text', function(t){
  ...
  const twoDaysFuture = superdate.dateFromSpeech(currentDate, 'in two days')
  t.equal(Math.floor((twoDaysFuture - currentDate)/86400000), 2)
  
  const threeMonthsFuture = superdate.dateFromSpeech(currentDate, 'in three months')
  t.equal(Math.round(((threeMonthsFuture - currentDate)/86400000)/10) * 10, 90)

  const threefoursevenDays = superdate.dateFromSpeech(currentDate, 'in three hundred forty seven days')
  t.equal(Math.floor((threefoursevenDays - currentDate)/86400000), 347)
  
  const threefoursevenDaysAgo = superdate.dateFromSpeech(currentDate, 'three hundred forty seven days ago')
  t.equal(Math.floor((threefoursevenDaysAgo - currentDate)/86400000), -347)
  
  const janOne = new Date(2015, 0, 1) // January 1, 2015 was a Thursday
  const nextWednesday = superdate.dateFromSpeech(janOne, 'next wednesday')
  t.equal(nextWednesday.getDate(), 7, 'should be the seventh of January')

  const janSeven = new Date(2015, 0, 7) // January 7, 2015 was a Wednesday
  const lastThursday = superdate.dateFromSpeech(janSeven, 'last thursday')
  t.equal(lastThursday.getDate(), 1, 'should be the first of January')
  
  const minusTwenty = superdate.dateFromSpeech(currentDate, 'minus twenty days')
  t.equal(Math.round((currentDate - minusTwenty)/86400000), 20)
  
  const zeroDays = superdate.dateFromSpeech(currentDate, 'zero days from now')
...
test('get date of first instance of day [in month] [in year]', function(t){
  t.plan(2)
  const dayDate = superdate.firstDayInstance(0, 0, 2014)
  t.equal(dayDate.getDate(), 5, 'should be 5 for first Sunday in Jan/2014')
  
  const nextDayDate = superdate.firstDayInstance(5, 0, 2014)
...
test('get date of last instance of day [in month] [in year]', function(t){
  t.plan(2)
  const dayDate = superdate.lastDayInstance(0, 0, 2014)
  t.equal(dayDate.getDate(), 26, 'should be 26 for last Sunday in Jan/2014')
  
  const nextDayDate = superdate.lastDayInstance(5, 0, 2014)
...
test('get prior instance of day', function(t){
   ...
   */
  const date = new Date(2015, 1, 5) // Feb. 5, 2015
...
test('get number of days remaining in year', function(t){
  ...
  t.plan(2)
  const date = new Date(2015, 11, 1) // Dec 1, 2015 (30 days left, 23 weekdays)
